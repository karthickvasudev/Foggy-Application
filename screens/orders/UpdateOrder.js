...
import {CreateOrderApi, GetListOfCustomer, GetListOfProducts, UpdateOrderApi} from "../apihelper/AppApi";
...
export default function UpdateOrder(props) {
                        ...
                        <HStack my={3} space={5}>
                            ...
                            <View width={16}>
                                <TextInput inputMode={"numeric"}
                                           ...
                                           value={String(orderLines[product.id]?.quantity ? orderLines[product.id]?.quantity : '')}
                                           onChange={(e) => {
                                               setOrderLines({
                                                   ...orderLines,
                                                   [product.id]: {
    ...
    const TotalAndCreateOrderBtn = ({
        ...
        const updateOrder = async () => {
            ...
            let response = await UpdateOrderApi(order.id, body);
            if (response.status === 200) {
                let json = await response.json();
                props.navigation.replace("View Order", json)
            } else {
                ToastAndroid.show("error in update order", 5)
            }
        }
        return <>
            <View style={[reuseStyle.stickyBottomButton, {backgroundColor: "white"}]} pb={3}>
                <View w={'100%'} px={5}>
                    ...
                    <PrimaryButton name={"Update Order"}
                                   onPress={() => updateOrder()}
    ...
    const MediatorCharge = ({mediatorCharge, setMediatorCharge}) => {
        return <>
            <HStack mt={3} alignSelf={"center"}>
                <View alignSelf={"center"}>
                    <Text bold>Mediator Charge : </Text>
                </View>
                <View>
                    <TextInput inputMode={"numeric"}
                               style={{
                               ...
                               value={String(mediatorCharge ? mediatorCharge : '')}
                               onChange={(e) => {
    ...
    const OrderForm = () => {
        ...
        useEffect(() => {
            setCustomerDetails({name: order.customer.name, phoneNumber: order.customer.phoneNumber})
            order.orderLines.forEach(line => {
                setOrderLines(prevState => {
                    return prevState = {
                        ...prevState,
                        [line.product.id]: {
                            ...prevState[line.product.id],
            ...
            if (order.orderType === 'MEDIATOR') {
                setMediatorCharge(order.mediatorPickupCharge)
            }
        }, []);
